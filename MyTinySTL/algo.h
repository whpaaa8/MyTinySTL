...
/*****************************************************************************************/
// splite
// 将字符串 按照 分隔符 进行分割,传入迭代器，用于存储分割的结果
/*****************************************************************************************/
template <class ForwardIter>
ForwardIter splite_n(string& str, string& delim, ForwardIter first,int n)
{
    char* buffer = new char[str.size() + 1];
    char* buffer_begin = buffer;
    char* _Str = const_cast<char*>(str.c_str());
    const char* _Delim = delim.c_str();
    char* tmp = strtok_s(_Str, _Delim, &buffer);
    while (tmp != nullptr && n--) {
        *first++ = tmp;
        tmp = strtok_s(nullptr, _Delim, &buffer);
    }
    delete[] buffer_begin;
    return first;
}
template <class ForwardIter>
ForwardIter splite(string& str, string& delim, ForwardIter first,ForwardIter last)
{
    char* buffer = new char[str.size() + 1];
    char* buffer_begin = buffer;
    char* _Str = const_cast<char*>(str.c_str());
...
/*****************************************************************************************/
// search_KMP 
// 在 [first1,last1) 区间寻找 [first2,last2) 第一次出现的位置
/*****************************************************************************************/
// [] 比 + 快很多！
template <class RandomAcessIter>
RandomAcessIter 
inline search_KMP(RandomAcessIter first1, RandomAcessIter last1,
           RandomAcessIter first2, RandomAcessIter last2)
{
    const int n1 = last1 - first1;
    const int n2 = last2 - first2;
    // if (n1*n2 < 10*(n1+n2)) return std::search(first1,last1,first2,last2);
    if (n1 < n2) return last1;
    int next[n2];
    next[0] = -1;
    int j = -1;
    for (int i = 1 ; i < n2; ++i)
    {
        while (j >= 0 && (first2[i] != first2[j+1])) j = next[j];
        if (first2[i] == first2[j+1]) ++j;
        next[i] = j;
    }
    j = -1;
    int i = 0;
    for (; i < n1 && j < n2-1; ++i)
    {
        while (j >= 0 && (first1[i] != first2[j+1])) j = next[j];
        if (first1[i] == first2[j+1]) ++j;
    }
    if (j == n2-1) return first1+i-n2;
    return last1;
}
char* search_KMP(char* first1, char*  last1,
...
template <class RandomAcessIter1,class RandomAcessIter2,class Compared>
RandomAcessIter1 
search_KMP(RandomAcessIter1 first1, RandomAcessIter1 last1,
           RandomAcessIter2 first2, RandomAcessIter2 last2,Compared comp)
{
    int n1 = last1 - first1;
    int n2 = last2 - first2;
    if (n1 < n2) return last1;
    ...
    for (int i = 1 ; i < n2; ++i)
    {
        while (j >= 0 && (!comp(first2[i],first2[j+1]))) j = next[j];
        if (first2[i] != first2[j+1]) ++j;
        next[i] = j;
    }
    j = -1;
    ...
    for (; i < n1 && j < n2-1; ++i)
    {
        while (j != -1 && (!comp(first1[i],first2[j+1]))) j = next[j];
        if (comp(first1[i],first2[j+1])) ++j;
    }
    if (j == n2-1) return first1 + i - n2;
    return last1;
}
template <class RandomAcessIter1,class RandomAcessIter2,class Size,class T>
RandomAcessIter1 
search_KMP_n(RandomAcessIter1 first1, RandomAcessIter1 last1,
           Size n, const T& value)
{
    int n1 = last1 - first1;
    if (n1 < n) return last1;
    int next[n];
    next[0] = -1;
    int j = -1;
    for (int i = 1 ; i < n; ++i)
    ...
    for (; i < n1 && j < n-1; ++i)
    {
        while (j != -1 && (first1[i] != value)) j = next[j];
        if (first1[i] == value) ++j;
    }
    if (j == n-1) return first1 + i - n;
    return last1;
}
template <class RandomAcessIter1,class RandomAcessIter2,class Size,
        class T, class Compared>
RandomAcessIter1 
search_KMP_n(RandomAcessIter1 first1, RandomAcessIter1 last1,
           Size n, const T& value,Compared comp)
{
    ...
    for (; i < n1 && j < n-1; ++i)
    {
        while (j != -1 && (!(comp(first1[i],value)))) j = next[j];
        if (comp(first1,value)) ++j;
    }
    if (j == n-1) return first1 + i - n;
    return last1;
}
