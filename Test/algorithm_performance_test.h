...
#define FUN_TEST3(mode, fun, count) do {                      \
    std::string fun_name = #fun;                               \
    srand((int)time(0));                                       \
    char buf[10];                                              \
    clock_t start, end;                                        \
    ...
    start = clock();                                           \
    for(size_t i = 0; i < count; ++i)                          \
         tmp = mode::fun(s1.begin(),s1.end(),s2.begin(),s2.end());    \
    end = clock();            \
...
void search_test()
{
  std::cout << "[----------------------- function : search ----------------------]" << std::endl;
  std::cout << "| orders of magnitude |";
  TEST_LEN(LEN1, LEN2, LEN3, WIDE);
  std::cout << "|         std         |";
  FUN_TEST3(std, search, LEN1);
  FUN_TEST3(std, search, LEN2);
  FUN_TEST3(std, search, LEN3);
  // std::cout << std::endl << "|        mystl        |";
  // FUN_TEST3(mystl, search, LEN1);
  // FUN_TEST3(mystl, search, LEN2);
  // FUN_TEST3(mystl, search, LEN3);
  std::cout << std::endl << "|         KMP         |";
  FUN_TEST3(mystl, search_KMP, LEN1);
  FUN_TEST3(mystl, search_KMP, LEN2);
  FUN_TEST3(mystl, search_KMP, LEN3);
  std::cout << std::endl;
}
