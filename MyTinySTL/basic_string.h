...
// 对wchar 类型的字符串的 偏特化萃取器： 不同的 处理函数
template <>
...
// 对char16_t 类型的字符串的 偏特化萃取器： 不同的 处理函数
template <>
...
// 对char32_t 类型的字符串的 偏特化萃取器： 不同的 处理函数
template <>
...
public:
  ...
  bool operator < (const basic_string& other) const{return compare(other) == -1;}
  bool operator > (const basic_string& other) const{return compare(other) == 1;}
